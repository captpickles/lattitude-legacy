...
use crate::netatmo::{NetatmoData, WeatherData};
use crate::{netatmo, purple, accuweather, calendar};
...
fn fetch_accuweather_daily_forecast() -> Pin<Box<dyn Future<Output=Result<Vec<DailyForecast>, anyhow::Error>>>> {
    Box::pin(
        async move {
...
fn fetch_accuweather_hourly_forecast() -> Pin<Box<dyn Future<Output=Result<Vec<HourlyForecast>, anyhow::Error>>>> {
    Box::pin(
        async move {
...
impl DataSource {
    ...
    async fn get_now(&self) -> Result<NowData, anyhow::Error> {
        ...
        if let Some(purple) = self.purple.get().await.ok() {
            now_data.aqi = purple
        }
